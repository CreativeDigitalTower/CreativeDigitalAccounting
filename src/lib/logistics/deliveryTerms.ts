...
// Alias-и за canonical дестинация (клиентът ползва „SKOPIE", не международното „Skopje").
const DEST_EN_OVERRIDE: Record<string, string> = { скопие: "SKOPIE", скопjе: "SKOPIE", skopje: "SKOPIE", тетово: "TETOVO", "бели извор": "BELI IZVOR" };
...
/**
 * Начални canonical дестинации (латиница/uppercase) за combobox-а (§4/§18) — на база
 * градовете/локациите на MK клиентите. Динамично се обогатява от Routes/Export Sets.
 */
export const MK_DESTINATIONS: string[] = [
  "SKOPIE", "PETROVEC", "GOSTIVAR", "KUMANOVO", "STRUMICA", "VINICA", "TETOVO",
  "KRIVA PALANKA", "SHTIP", "LIPKOVO", "KOCHANI",
  "DOLNO KONJARI", "RZANICINO", "BATINCI", "JAKIMOVO", "NIKUSHTAK", "ORIZARI", "VIZBEGOVO", "BUTEL",
];

/** Нормализиран ключ за dedupe на дестинации (fold кирилица→латиница, само буквено-цифрови). */
export function normalizeDestination(v: string | null | undefined): string {
  return destinationEn(v).toLowerCase().replace(/[^a-z0-9]/g, "");
}

/**
 * Обединява canonical + динамични (routes/export sets/client cities) дестинации в един
 * уникален списък (dedupe по normalizeDestination), пазейки canonical латиница (§6).
 */
export function mergeDestinations(...sources: (string | null | undefined)[][]): string[] {
  const byKey = new Map<string, string>();
  for (const src of sources) {
    for (const raw of src) {
      const val = (raw ?? "").trim();
      if (!val) continue;
      const key = normalizeDestination(val);
      if (!key) continue;
      // Предпочитаме canonical латиница; ако още няма стойност за ключа → записваме.
      if (!byKey.has(key)) byKey.set(key, destinationEn(val) || val.toUpperCase());
    }
  }

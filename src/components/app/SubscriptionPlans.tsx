...
export function SubscriptionPlans({ currentPlan, trialUsed, bank }: { currentPlan: string; trialUsed: boolean; bank: Bank }) {
  ...
  async function startTrial(planId: string) {
    ...
    if (res.ok) {
      ...
      router.refresh();
    } else setTrialMsg((await res.json()).error ?? t("pricing.sub.trialError"));
  }
    ...
    <div>
        ...
        {PLAN_DETAILS.map((plan) => {
          ...
          return (
            <div key={plan.id} className="glass panel"
              ...
              {isCurrent && <span style={{ position: "absolute", top: -10, left: 16, background: "var(--emerald)", color: "#fff", fontSize: 10.5, fontWeight: 600, padding: "3px 10px", borderRadius: 20 }}>{t("pricing.active")}</span>}
              {!isCurrent && plan.recommended && <span className="ribbon">{t("pricing.recommended")}</span>}
              {!isCurrent && hasPromo && <span style={{ position: "absolute", top: 10, left: 10, background: "var(--brick)", color: "#fff", fontSize: 9.5, fontWeight: 700, padding: "2px 8px", borderRadius: 20 }}>{t("pricing.specialPrice")}</span>}
              ...
              <div style={{ fontFamily: "'Fraunces', serif", fontSize: 19, fontWeight: 700, textAlign: "center" }}>{pt.name}</div>
              <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 2, minHeight: 32, textAlign: "center" }}>{pt.tagline}</div>
              ...
              <div style={{ margin: "10px 0 0", textAlign: "center" }}>
                ...
                <span style={{ fontSize: 13, color: "var(--muted)", fontWeight: 500 }}> {t("pricing.perMonth")}</span>
              </div>
              ...
              <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 12, fontStyle: "italic" }}>{t("pricing.suitedFor", { text: pt.suited })}</div>

              {isCurrent ? (
                <button className="btn btn-ghost" style={{ width: "100%", justifyContent: "center" }} disabled>{t("pricing.sub.currentPlan")}</button>
              ) : plan.price === 0 ? (
                <button className="btn btn-ghost" style={{ width: "100%", justifyContent: "center" }} disabled>{t("pricing.sub.freeBtn")}</button>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  <button onClick={() => choosePay(plan.id)} className={plan.recommended ? "btn btn-primary" : "btn btn-ghost"} style={{ width: "100%", justifyContent: "center" }}>
                    {currentPlan === "free" ? t("pricing.sub.upgrade") : t("pricing.sub.changePlan")}
                  </button>
      ...
      {promo && (
        <div className="glass panel" style={{ marginTop: 16, padding: "14px 20px", textAlign: "center", borderLeft: "4px solid var(--brick)", background: "var(--brick-soft)", fontSize: 13 }}>
          <strong style={{ color: "var(--brick)" }}>{t("pricing.promoStrong")}</strong> {t("pricing.promoText")}
        </div>
      )}
      ...
      <div id="pay-box" className="glass panel" style={{ marginTop: 16, padding: "20px 24px", borderLeft: "4px solid var(--emerald)" }}>
        <h3 style={{ fontFamily: "'Fraunces', serif", fontSize: 17, margin: "0 0 6px", display: "flex", alignItems: "center", gap: 8 }}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" style={{verticalAlign:"-2px"}}><rect x="2.5" y="5" width="19" height="14" rx="2.5"/><path d="M2.5 9.5h19M6 15h4"/></svg> {t("pricing.sub.bankTitle")}</h3>
        <p style={{ fontSize: 12.5, color: "var(--ink-soft)", margin: "0 0 14px", maxWidth: 700 }}>
          {payPlan
            ? <>{t("pricing.sub.bankChosenPrefix")}<strong>{P.plans[payPlan.id].name}</strong>{t("pricing.sub.bankChosenSuffix", { period: t(`pricing.periods.${period.id}`).toLowerCase() })}</>
            : <>{t("pricing.sub.bankIntroEmpty")}</>}
        </p>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px,1fr))", gap: 12 }}>
          {[
            { label: t("pricing.sub.bankRecipient"), value: bank.recipient },
      ...
      {payModal && payPlan && createPortal(
        <div onClick={() => setPayModal(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.5)", display: "flex", alignItems: "flex-start", justifyContent: "center", zIndex: 3000, padding: "6vh 16px 16px", overflowY: "auto" }}>
          <div onClick={(e) => e.stopPropagation()} className="glass panel" style={{ width: "min(560px, 100%)", padding: 28, margin: "auto", borderTop: "4px solid var(--emerald)" }}>
            ...
            <h3 style={{ fontFamily: "'Fraunces', serif", fontSize: 20, margin: "0 0 6px", textAlign: "center" }}>{t("pricing.sub.modalTitle")}</h3>
            <p style={{ fontSize: 13, color: "var(--ink-soft)", textAlign: "center", margin: "0 0 18px", lineHeight: 1.5 }}>
              {t("pricing.sub.modalIntroPrefix")}<strong>{P.plans[payPlan.id].name}</strong>{t("pricing.sub.modalIntroSuffix", { period: t(`pricing.periods.${period.id}`).toLowerCase() })}<br />
              {t("pricing.sub.modalIntro2")}
            </p>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 14, flexWrap: "wrap" }}>
              {proformaLoading ? (
                <button className="btn btn-ghost" style={{ flex: 1, justifyContent: "center", minWidth: 200 }} disabled>{t("pricing.sub.genProforma")}</button>
              ) : proforma ? (
                <a href={`/d/${proforma.token}`} target="_blank" rel="noopener noreferrer" className="btn btn-ghost" style={{ flex: 1, justifyContent: "center", minWidth: 200 }}>
                  ...
                  {t("pricing.sub.downloadProforma")}
                </a>
              ) : null}
            ...
            {proforma && <p style={{ fontSize: 11.5, color: "var(--muted)", marginTop: 8, textAlign: "center" }}>{t("pricing.sub.proformaAvail", { number: proforma.number })}</p>}
          </div>
        </div>,
        document.body
      )}

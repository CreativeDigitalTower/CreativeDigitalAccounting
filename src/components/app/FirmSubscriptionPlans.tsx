...
export function FirmSubscriptionPlans({ currentPlan }: { currentPlan: string | null }) {
  ...
  const FP = (messages as unknown as { pricing: { firmFeatures: string[]; firmPlans: Record<string, { name: string; tagline: string }> } }).pricing;
        ...
        {ACCOUNTANT_PLANS.map((p) => {
          ...
          return (
            <div key={p.id} className="glass panel" style={{ padding: "22px 20px", position: "relative", border: isCurrent ? "2px solid var(--emerald)" : p.recommended ? "2px solid var(--brass)" : undefined, display: "flex", flexDirection: "column" }}>
              ...
              {!isCurrent && p.recommended && <span className="ribbon">{t("pricing.recommended")}</span>}
              <div style={{ fontFamily: "'Fraunces', serif", fontSize: 18, fontWeight: 700, textAlign: "center" }}>{FP.firmPlans[p.id]?.name ?? p.name}</div>
              <div style={{ fontSize: 12, color: "var(--muted)", textAlign: "center", minHeight: 32, marginTop: 2 }}>{FP.firmPlans[p.id]?.tagline ?? p.tagline}</div>
              <div style={{ textAlign: "center", margin: "10px 0 0" }}>
                {p.custom ? (
                  <span className="num" style={{ fontSize: 15, fontWeight: 700 }}>{t("pricing.byArrangement")}</span>
                ) : (
                  <>
                    ...
                    <span style={{ fontSize: 12, color: "var(--muted)" }}> {t("pricing.perMonthShort")}</span>
                  </>
                )}
              </div>

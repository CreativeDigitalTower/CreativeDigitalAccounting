...
export function FirmClientBanner({ companyName }: { companyName: string }) {
  ...
  return (
    <div className="no-print" style={{ background: "var(--navy)", color: "#fff", padding: "8px 20px", display: "flex", alignItems: "center", gap: 12, fontSize: 13, flexWrap: "wrap" }}>
      ...
      <span>Работите по фирма <strong>{companyName}</strong></span>
      <button onClick={exit} className="btn btn-sm" style={{ marginLeft: "auto", background: "rgba(255,255,255,.16)", color: "#fff", border: "1px solid rgba(255,255,255,.3)" }}>← Всички клиенти</button>
    </div>
  );
}

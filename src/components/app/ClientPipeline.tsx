        ...
        return (
          <div key={st.id}
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <span style={{ fontWeight: 700, fontSize: 13 }}>{t(`clients.stage.${st.id}`)}</span>
              ...
              {list.map((c) => (
                <div key={c.id} draggable onDragStart={() => setDrag(c.id)} onDragEnd={() => setDrag(null)}
                  ...
                  <div style={{ fontSize: 11.5, color: "var(--muted)", marginTop: 3 }}>
                    {c.dealValue ? t("clients.pipeline.deal", { v: formatCurrency(c.dealValue) }) : t("clients.pipeline.invoiced", { v: formatCurrency(c.total) })}
                  </div>
                </div>
              ))}
            </div>

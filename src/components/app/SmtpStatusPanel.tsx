"use client";
import { useEffect, useState } from "react";
import { useT } from "@/components/i18n/I18nProvider";

type Config = { configured: boolean; host: string | null; port: number; secure: boolean; user: string | null; from: string; replyTo: string; hasPassword: boolean };
type Status = { config: Config; verify: { ok: boolean; error?: string } } | null;

export function SmtpStatusPanel() {
  const t = useT();
  const [status, setStatus] = useState<Status>(null);
  const [loading, setLoading] = useState(true);
  const [testTo, setTestTo] = useState("");
  const [sending, setSending] = useState(false);
  const [result, setResult] = useState<{ ok: boolean; msg: string } | null>(null);

  async function load() {
    setLoading(true);
    try {
      const r = await fetch("/api/admin/email-test");
      setStatus(await r.json());
    } catch { setStatus(null); }
    setLoading(false);
  }
  useEffect(() => { load(); }, []);

  async function sendTest() {
    setSending(true); setResult(null);
    try {
      const r = await fetch("/api/admin/email-test", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify(testTo ? { to: testTo } : {}),
      });
      const d = await r.json();
      if (d.ok) setResult({ ok: true, msg: t("admintools.smtp.testSent", { to: d.to }) });
      else setResult({ ok: false, msg: t("admintools.smtp.testFail", { status: d.status, error: d.error || t("admintools.smtp.noDetails") }) });
    } catch (e) {
      setResult({ ok: false, msg: t("admintools.smtp.reqError", { e: String(e) }) });
    }
    setSending(false);
  }

  const c = status?.config;
  const v = status?.verify;
  const online = !!v?.ok;

  return (
    <div className="glass panel" style={{ padding: "20px 24px", marginBottom: 18 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
        <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: 17, fontWeight: 600, margin: 0 }}>{t("admintools.smtp.title")}</h2>
        {loading ? (
          <span style={{ fontSize: 12.5, color: "var(--muted)" }}>{t("admintools.smtp.checking")}</span>
        ) : (
          <span style={{ display: "inline-flex", alignItems: "center", gap: 7, fontSize: 12.5, fontWeight: 700, color: online ? "var(--emerald-dark)" : "var(--brick)" }}>
            <span style={{ width: 9, height: 9, borderRadius: "50%", background: online ? "var(--emerald)" : "var(--brick)", display: "inline-block" }} />
            {online ? t("admintools.smtp.ok") : c?.configured ? t("admintools.smtp.err") : t("admintools.smtp.notConfigured")}
          </span>
        )}
      </div>

      {!loading && c && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: "8px 18px", fontSize: 12.5, marginBottom: 14 }}>
          <div><span style={{ color: "var(--muted)" }}>Host:</span> {c.host ?? "—"}</div>
          <div><span style={{ color: "var(--muted)" }}>Port:</span> {c.port} {c.secure ? "(SSL/TLS)" : "(STARTTLS)"}</div>
          <div><span style={{ color: "var(--muted)" }}>{t("admintools.smtp.user")}:</span> {c.user ?? "—"}</div>
          <div><span style={{ color: "var(--muted)" }}>{t("admintools.smtp.password")}:</span> {c.hasPassword ? t("admintools.smtp.pwSet") : t("admintools.smtp.pwMissing")}</div>
          <div style={{ gridColumn: "1 / -1" }}><span style={{ color: "var(--muted)" }}>From:</span> {c.from}</div>
          <div style={{ gridColumn: "1 / -1" }}><span style={{ color: "var(--muted)" }}>Reply-To:</span> {c.replyTo}</div>
        </div>
      )}

      {!loading && v && !v.ok && v.error && (
        <div style={{ background: "var(--brick-soft)", border: "1px solid var(--brick)", color: "var(--brick)", borderRadius: 8, padding: "9px 13px", fontSize: 12.5, marginBottom: 14 }}>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" style={{verticalAlign:"-2px"}}><path d="M12 3 1.5 21h21L12 3Z"/><path d="M12 10v5M12 18h.01"/></svg> {v.error}
        </div>
      )}

      <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
        <input type="email" value={testTo} onChange={(e) => setTestTo(e.target.value)} placeholder={c?.user ?? t("admintools.smtp.testPh")} style={{ flex: "1 1 220px", minWidth: 200 }} />
        <button onClick={sendTest} disabled={sending} className="btn btn-primary btn-sm">{sending ? t("admintools.smtp.sending") : t("admintools.smtp.sendTest")}</button>
        <button onClick={load} disabled={loading} className="btn btn-ghost btn-sm">{t("admintools.smtp.recheck")}</button>
      </div>

      {result && (
        <div style={{ marginTop: 12, fontSize: 12.5, fontWeight: 600, color: result.ok ? "var(--emerald-dark)" : "var(--brick)" }}>
          {result.ok ? "✓ " : "✕ "}{result.msg}
        </div>
      )}
    </div>
  );
}

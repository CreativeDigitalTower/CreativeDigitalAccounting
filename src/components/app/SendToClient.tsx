...
export function SendToClient({ id, defaultEmail, decision, sentAt }: {
  ...
  async function send() {
    ...
    setBusy(false);
    if (res.ok) { setDone(t("documents.send.sent")); setTimeout(() => setOpen(false), 1500); }
    else setDone(t("documents.send.errSend"));
  }
      ...
      {open && (
        <div onClick={() => setOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 16 }}>
          <div onClick={(e) => e.stopPropagation()} className="glass panel" style={{ width: 380, maxWidth: "100%", padding: 22, borderRadius: 14, boxShadow: "0 12px 40px rgba(0,0,0,.18)" }}>
            ...
            <label style={{ fontSize: 12.5, fontWeight: 600, display: "block", marginBottom: 6 }}>{t("documents.send.emailLabel")}</label>
            ...
            <p style={{ fontSize: 11.5, color: "var(--muted)", margin: "0 0 14px" }}>{t("documents.send.hint")}</p>
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <button onClick={send} disabled={busy} className="btn btn-primary btn-sm">{busy ? t("documents.send.sending") : t("documents.send.send")}</button>
            ...
            {sentAt && <p style={{ fontSize: 11, color: "var(--muted)", marginTop: 10 }}>{t("documents.send.lastSent", { when: new Date(sentAt).toLocaleString(locale) })}</p>}
          </div>
        </div>
      )}
    </>

...
import { hasLogisticsAccess } from "@/lib/logistics/access";
...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const [company, me, jar, sub, inboxUnread, logisticsEnabled, myCompanyRows] = await Promise.all([
    ...
    hasLogisticsAccess(companyId).catch(() => false),
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", position: "relative" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, display: "flex", width: "100%" }}>
        <SidebarShell companyName={company.name} plan={plan} isSuperAdmin={isSuperAdmin} logoUrl={plan !== "free" ? company.logoUrl : null} inboxUnread={inboxUnread} companyEik={company.eik} companies={myCompanyRows} activeCompanyId={companyId} logisticsEnabled={logisticsEnabled} />

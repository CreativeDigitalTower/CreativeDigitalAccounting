  ...
  const Party = ({ title, name, eik, address, mol }: { title: string; name?: string | null; eik?: string | null; address?: string | null; mol?: string | null }) => (
    <div style={{ flex: 1 }}>
      ...
      <div style={{ fontSize: 12.5, color: "var(--ink-soft)", lineHeight: 1.7 }}>
        {eik && <div>{t("subdocs.prot.doc.eik")} {eik}</div>}
        {address && <div>{address}</div>}
        {mol && <div>{t("subdocs.prot.doc.mol")} {mol}</div>}
      </div>
    </div>
  );
    ...
    <>
      ...
      <div className="glass printable" style={{ borderRadius: 14, maxWidth: 800, padding: "40px 48px" }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <div style={{ fontFamily: "'Fraunces', serif", fontSize: 24, fontWeight: 700 }}>{t("subdocs.prot.doc.title")}</div>
          <div className="num" style={{ fontSize: 14, color: "var(--ink-soft)", marginTop: 4 }}>{t("subdocs.prot.doc.numberPrefix")} {p.number}</div>
          <div style={{ fontSize: 12.5, color: "var(--ink-soft)", marginTop: 4 }}>
            {p.place ? `${p.place}, ` : ""}{new Date(p.date).toLocaleDateString(locale)}
          </div>
        </div>

        <div style={{ display: "flex", gap: 24, marginBottom: 24, borderBottom: "1px solid var(--border)", paddingBottom: 20 }}>
          <Party title={t("subdocs.prot.doc.handsOver")} name={company?.name} eik={company?.eik} address={[company?.address, company?.city].filter(Boolean).join(", ")} mol={company?.mol} />
          <Party title={t("subdocs.prot.doc.receives")} name={p.counterpartyName} eik={p.counterpartyEik} address={p.counterpartyAddress} mol={p.counterpartyMol} />
        </div>

        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 11.5, color: "var(--muted)", fontWeight: 600, letterSpacing: 1, marginBottom: 8 }}>{t("subdocs.prot.doc.subject")}</div>
          <div style={{ fontSize: 13.5, lineHeight: 1.7, whiteSpace: "pre-wrap" }}>{p.items ?? "—"}</div>
        </div>

        {p.description && (
          <div style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 11.5, color: "var(--muted)", fontWeight: 600, letterSpacing: 1, marginBottom: 8 }}>{t("subdocs.prot.doc.notesLabel")}</div>
        ...
        <div style={{ display: "flex", gap: 24, marginTop: 48 }}>
          <div style={{ flex: 1, textAlign: "center" }}>
            <div style={{ borderTop: "1px solid var(--ink)", paddingTop: 6, fontSize: 12.5 }}>{t("subdocs.prot.doc.handed")} {p.handedBy ?? "................"}</div>
          </div>
          <div style={{ flex: 1, textAlign: "center" }}>
            <div style={{ borderTop: "1px solid var(--ink)", paddingTop: 6, fontSize: 12.5 }}>{t("subdocs.prot.doc.received")} {p.receivedBy ?? "................"}</div>
          </div>
        </div>

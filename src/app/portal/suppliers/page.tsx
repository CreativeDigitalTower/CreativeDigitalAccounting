...
export default async function PortalSuppliersPage() {
  ...
  return (
    <div className="glass panel">
      <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: 20, fontWeight: 700, margin: "0 0 4px" }}>Доставчици</h1>
      <p style={{ fontSize: 12, color: "var(--muted)", margin: "0 0 14px" }}>Списък само за преглед.</p>
      {suppliers.length === 0 ? <div style={{ fontSize: 13, color: "var(--muted)" }}>Няма доставчици.</div> : (
        <table>
          <thead><tr><th>Доставчик</th><th>Град</th></tr></thead>
          <tbody>
            {suppliers.map((s) => (
